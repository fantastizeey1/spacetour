...
const Destination = () => {
 ...
 // Run only once on mount
  ...
  return (
    ...
    <div className='h-svh flex flex-col items-center justify-center w-full overflow-hidden  bg-cover lg:justify-start lg:items-start px-5 lg:p-0 mb-4'
    style={{ backgroundImage: `url('${backgroundImage}')` }} >
      ...
      <div className='flex flex-col justify-center items-center mt-4 font-barlow text-white md:items-start lg:mt-10 lg:mx-auto '>
        ...
        <div className='lg:w-[445px]'>
          <img src={planetImages[planetNumber]}  alt="" className='w-[170px] md:w-[300px] lg:w-[380px]' 
          />
        </div>
        ...
        <div onClick={() => handlePlanetChange(0)} className={`dest-tab mx-3 md:mx-6 lg:ml-0 cursor-pointer ${planetNumber === 0 ? 'active' : ''}`}>
         MOON
        </div>
        <div onClick={() => handlePlanetChange(1)} className={`dest-tab mx-3 md:mx-6 cursor-pointer ${planetNumber === 1 ? 'active' : ''}`}>
        MARS
        </div>
        <div onClick={() => handlePlanetChange(2)} className={`dest-tab mx-3 md:mx-6 cursor-pointer ${planetNumber === 2 ? 'active' : ''}`}>
         EUROPA
        </div>
        <div onClick={() => handlePlanetChange(3)} className={`dest-tab mx-3 md:mx-6 cursor-pointer ${planetNumber === 3 ? 'active' : ''}`}>
         TITAN
        </div>
           ...
           <div className='flex flex-col items-center my-1 lg:items-start'>
              ...
              <h3 className='text-[28px] font-bellefair uppercase'>{data.destinations[planetNumber].distance}</h3>
            </div>
            <div className='flex flex-col items-center my-1'>
              <p className='text-[14px] text-[#89898a]'>EST. TRAVEL TIME</p>
              <h3 className='text-[28px] font-bellefair uppercase'>{data.destinations[planetNumber].travel}</h3>
            </div>
           </div>
          </div>
        </div>
